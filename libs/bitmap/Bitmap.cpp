...
bool Bitmap::loadBMP(char *file) {
	...
	int foo = fread(&bmfh, sizeof(BitmapFileHeader), 1, in);
	...
	foo = fread(&bmih, sizeof(BitmapInfoHeader), 1, in);
	...
	if (bpp == 8) {
		...
		foo = fread(colours, sizeof(RGBQuad), numColours, in);
	}
	...
	foo = fread(tempData, sizeof(char), dataSize, in);
