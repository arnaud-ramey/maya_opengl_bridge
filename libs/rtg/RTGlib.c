...
int RTG_ParseFlag(const char *line) {
	...
	printf("Parse error: Flag '%s' contains no on/off selector\n", line);
	return 0;
}
...
int RTG_ParseFlags(void) {
	...
	if (end == -1) {
				...
				= RTG_FindNextLine(0, Parser.lines, Parser.nlines,
						"HIERARCHY_LIST");
		if (end == -1) {
			printf("Parse error: Identifier 'MATERIAL_LIST' not found\n");
			return 0;
		}
	}
...
int RTG_ParseMaterials(void) {
	...
	if (nmat < 0 || nmat > RTG_MAX_MATERIALS) {
		printf("Too many materials. Maximum allowed is %d\n", RTG_MAX_MATERIALS);
		return 0;
	}
	...
	for (i = 0; i < nmat; i++) {
		...
		if (mline == -1) {
			printf("Parse error: Identifier '%s' not found.\n", tmp);
			return 0;
		}
		++mline;
...
void RTG_AddChild(RTGNode *parent, RTGNode *child) {
	if (parent->nchildren == RTG_MAX_CHILDREN) {
		printf("Node cannot have more than %d children: ignored.\n",
				RTG_MAX_CHILDREN);
		return;
	}
...
int RTG_ParseHierarchy(void) {
	...
	if (ntop > RTG_MAX_TOP_LEVEL_NODES) {
		printf("Too many top-level nodes. Maximum allowed is %d\n",
	...
	for (i = 0; i < ntop; i++) {
		if (RTG_BeginsWithIgnoreWhitespace(Parser.lines[l],
			...
			printf("Parse error: Malformed hierarchy description\n");
			return 0;
		}
...
int RTG_ParseObject(RTGObject *obj, int start) {
	...
	if (Parser.flags[OUTPUT_MATERIALS]) {
		if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "USES_MATERIAL")) {
			printf("Parse error: Identifier 'USES_MATERIAL' not found.\n");
			return -1;
		}
	...
	if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "VERTEX")) {
		printf("Parse error: Identifier 'VERTEX' not found\n");
		return -1;
	}
	++p;
	for (i = 0; i < nvert; i++) {
	...
	if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "NORMAL")) {
		printf("Parse error: Identifier 'NORMAL' not found\n");
		return -1;
	}
	++p;
	for (i = 0; i < nnorm; i++) {
	...
	if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "TEXCOORD")) {
		printf("Parse error: Identifier 'TEXCOORD' not found\n");
		return -1;
	}
	++p;
	for (i = 0; i < ntc; i++) {
	...
	if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "POLYGON")) {
		printf("Parse error: Identifier 'POLYGON' not found\n");
		return -1;
	}
	++p;
	for (i = 0; i < npoly; i++) {
		...
		if (nvert > RTG_MAX_POLY_VERT) {
			printf("Polygon has %d vertices, maximum number allowed is %d\n",
...
int RTG_ParseObjects(unsigned int nobj) {
	...
	for (i = 0; i < nobj; i++) {
		if (!RTG_BeginsWithIgnoreWhitespace(Parser.lines[p], "OBJECT_START")
				|| p >= (int) Parser.nlines) {
			printf("Parse error: Identifier 'OBJECT_START' not found\n");
			return 0;
		}
		if (Parser.verbose) {
			printf("Object %d\n", i);
