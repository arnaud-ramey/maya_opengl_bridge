 ...
 */
void read_texture_from_file(string filename, int id_tex) {
    cout << endl << "-> read_texture_from_file(" << filename << ", " << id_tex
         << ")" << endl;
    create_texture_begin(id_tex);

    Bitmap* img = new Bitmap((char*) filename.c_str());
 ...
 * @return
 */
int find_texture_index(string filename) {
 ...
 * @param idTexUnit
 */
void load_texture(string filename_str, int idTexUnit) {
    // remove the windows path
    size_t found = filename_str.find(TEXTURES_WINDOWS_FOLDER);
    if (found != string::npos)
        filename_str.replace(found, strlen(TEXTURES_WINDOWS_FOLDER), "");

    ostringstream filename_full;
    filename_full << TEXTURES_FOLDER << filename_str;
    filename_str = filename_full.str();

    int index_in_vector = find_texture_index(filename_str);
    /*
    ...
    if (index_in_vector == -1) {
        ...
        cout << "   We will read the texture '" << filename_str << "' in slot "
             << first_available_texture_index << endl;
        cout << "   " << textures.size() << " already loaded textures :"
             << endl;
        if (textures.size() > 0)
            for (vector<Tex>::iterator tex = textures.begin(); tex
                 != textures.end(); ++tex)
        ...
        read_texture_from_file(filename_str, first_available_texture_index);
